...
//        $carreras=$repository->getRepository('minsalSIGminsalSIGBundle:SsCarrera')->findAll();      	 
        ...
        if(count($carreras)>0){       	 
            ...
            foreach($carreras as $carrera){ // $carreras arreglo <-- $carrera valor individual           	 
                if($carrera->getId()==1 or $carrera->getId()==2)    
        ...
        if ($form->isValid()) {
            ...
            if($c_Id == 0){
                ...
                if($u_Id == 0){                                                   
                    ...
                    $resultado = $conn->fetchAll('SELECT ci.id, ci.id_carrera, ci.id_institucion_formadora, sum(e.limite_superior) monto
                                                    FROM ss_carrera_inst_form ci 
                                                        LEFT JOIN (select c.limite_superior, e1.id_carrera_inst_form
                                                                    from ss_categoria c, ss_estudiante e1
                                                                    where c.id=e1.id_categoria) e
                                                        ON(ci.id=e.id_carrera_inst_form)
                                                    WHERE ci.id_carrera = 1 OR ci.id_carrera = 2	
                                                    GROUP BY ci.id, ci.id_carrera, ci.id_institucion_formadora
                                                    ORDER BY ci.id');
                                                                       
                    return $this->render('minsalSIGminsalSIGBundle:Estrategico:montoTotal1.html.twig', array('carreras' => $carreras, 'universidades' => $universidad, 'resultados'=>$resultado));                                                                                     
                }
                else 
                        ...
                        $resultado = $conn->fetchAll('SELECT ci.id, ci.id_carrera, ci.id_institucion_formadora, sum(e.limite_superior) monto
                                                        FROM ss_carrera_inst_form ci 
                                                                LEFT JOIN (select c.limite_superior, e1.id_carrera_inst_form
                                                                                ...
                                                                                where c.id=e1.id_categoria) e
                                                                ON(ci.id=e.id_carrera_inst_form)
                                                        WHERE ci.id_institucion_formadora= '.$u_Id.' AND (ci.id_carrera = 1 OR ci.id_carrera = 2)
                                                        GROUP BY ci.id, ci.id_carrera, ci.id_institucion_formadora
                                                        ORDER BY ci.id');
                                                                       
                        return $this->render('minsalSIGminsalSIGBundle:Estrategico:montoTotal2.html.twig', array('carreras' => $carreras, 'universidad' => $universidadId, 'resultados'=>$resultado)); 
                    }
            }
            //Selecciona una carrera
                ...
                if($c_Id != 0){
                    ...
                    if($u_Id == 0){
                        ...
                        return $this->render('minsalSIGminsalSIGBundle:Estrategico:montoTotal3.html.twig', array('carrera' => $carreraId, 'universidades' => $universidad, 'resultados'=>$resultado)); 
                    }
                        ...
                        if($u_Id != 0){  
                            ...
                            return $this->render('minsalSIGminsalSIGBundle:Estrategico:montoTotal4.html.twig', array('carrera' => $carreraId, 'universidad' => $universidadId, 'resultados'=>$resultado)); 
                        }
                }        
        }
             
             
             return $this->render('minsalSIGminsalSIGBundle:Estrategico:montoTotal.html.twig', array('form' => $form->createView()));
